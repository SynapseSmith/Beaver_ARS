...
class Args:
    def __init__(self):
        ...
        self.response_templates = {
            ...
            1: [   # 특정 메뉴 안내
                "{STD_CATEGORY_NM}의 가격은 {PRICE}입니다. 더 자세한 내용은 문자로 발송해드린 메뉴판을 확인해주세요."
            ],
            2: [   # 인기 / 추천
                "{INDI_TYPE_NM1} 메뉴를 {RECOMMEND_MENU} 메뉴로서 추천드려요."
            ],
            3: [    # 계절 한정 메뉴 (예: 여름 특선, 겨울 메뉴) / 프로모션 메뉴 (예: 신메뉴 할인) 안내 / 신메뉴
                "{LIMMIT_MENU} 메뉴가 {INDI_TYPE_NM3} 메뉴에요."  # 메뉴 3개!!!!!!!!!!!!!!!!!!!    /     # 개선 필요함. 이 부분까지는 데이터 생성 못해서 나중에.
            ],
            4: [   # 주문 / 전달 방식 안내 (키오스크, 테이블오더, 스마트주문 / 내점, 포장, 배달 등)
                "{YN}해요. 저희는 {ORDER_TYPE} 방식의 주문이 가능 해요"
            ],   # YN: "네 가능"/"아니오 불가능"
            5: [ # 결제 방법 안내(현금, 카드, 간편 결제등)
                "{YN}}해요. 이외에 {결제수단1},{결제수단2},{결제수단3}으로 결제 가능해요"
            ],   # YN: "네 가능"/"아니오 불가능"
            6: [  # 특정 결제 수단 안내
                # !!!!!!!!!!!!!!!!!!!
            ],
            7: [   # 영업 시작/종료 시간 안내
                "{월/일}은 #{시}에 영업이 종료돼요. 저희 {상점명}의 영업시간은 {00}시 {00}분 부터 {00}시 {00}분 까지에요"
            ],
            8: [   # 특정 요일 영업 시작/종료 시간 안내
                # !!!!!!!!!!!!!!!!!!!!
            ],
            9: [   # 정기 휴무/주말/공휴일 운영 여부 안내
                "네 맞아요/ 아니에요. 저희 {상점명}의 휴일은 매주{요일}이에요."
            ]
            , 
            10: [   # 배달 가능 지역 안내
            ...
            11: [   # 배달비 및 최소 주문 금액 안내
                "저희 {STORE_NM}의 배달 팁은 최소 {DLVR_TIP_AMT} 이며 {ORDER_BGN_AMT} 이상 주문해주셔야 배달이 가능해요/"
            ],
            12: [  # 예약 가능 여부 안내(전화/온라인)
