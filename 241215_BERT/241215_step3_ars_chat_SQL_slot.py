    ...
    def __init__(self):
        ...
        self.response_templates = {
            ...
            6: [   # PAYMENT_AVAIL 처리하기
                "{PAYMENT_AVAIL} 저희 매장은 {PAYMNT_MN_CD} 방식의 결제가 가능해요."
            ],
            7: [
    ...
    try:
        ...
        if intent_id in [1, 6, 8]:
            ...
            if intent_id == 6:  # 가능 여부 판단을 위해서 임계값 이상인 게 있는 경우는 있다고 판단, 없는 경우 없다고 판단.
                extracted_info = get_best_match(user_message, select_info, yn=True)
                if extracted_info:
...
if __name__ == '__main__':
    app.run(host='0.0.0.0', port=1119, debug=False)
