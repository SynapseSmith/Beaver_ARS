...
class args:
    ...
    llama_v1_model_name_or_path ="MLP-KTLim/llama-3-Korean-Bllossom-8B" #"MLP-KTLim/llama-3-Korean-Bllossom-8B" #"NCSOFT/Llama-VARCO-8B-Instruct" #"Bllossom/llama-3.1-Korean-Bllossom-Vision-8B"
    instruction_template = "/home/user09/beaver/data/shared_files/prompt/incontext_v4.txt"
...
retriever_dict = {
    f"{args.store}": get_retriever(read_db(args.output_dir, "faiss", f"{args.store}", hf), read_db(args.output_dir, "docs", f"{args.store}"))
}
...
def initialize_model():
    global prompt_chain, llm_chain, streamer, model, tokenizer  # 함수 밖에서도 변수의 유효범위를 유지하기 위해, 전역변수로 선언.
    ...
    prompt_chain = PromptTemplate(  # 프롬프트 템플릿을 채워주는 역할.
        input_variables=["store", "all_menu", "current_input", "hongkong"],
...
def order():
    try: 
        global is_initial, endoforder, model, llm_chain, logger, is_first, is_response_yielding, message, tokenizer
        ...
        # @stream_with_context
        def generate():
            global is_initial, endoforder, is_first, tokenizer
            ...
            # retriever_result = format_docs([f"{i.page_content}: 옵션: {i.metadata['옵션']}, 특징: {i.metadata['특징']}" for i in retriever.invoke(invoke_format(inputs))])
            retriever_result = format_docs([f"{i.page_content}" for i in retriever.invoke(invoke_format(inputs))])
            print("retriever_result:\n", retriever_result)
            inputs['hongkong'] = retriever_result
...
if __name__ == '__main__':
    app.run(host='0.0.0.0', port=5010, debug=False)   #5000포트에서 요청을 받음. 따라서 클라이언트에서는 해당 로컬 서버의 IP주소에서 포트 넘버 5006으로 보내야함.