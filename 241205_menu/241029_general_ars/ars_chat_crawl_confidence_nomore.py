...
class args:
    store = '103807'   # 102496(히스피커피), 102506(삼청당), 103807(노모어피자), 104570(롤링파스타), 104933(홍콩반점)
    ...
    instruction_template = "/home/user09/beaver/data/shared_files/prompt/incontext_v8_juju.txt"
...
def read_db(output_dir, db_type, name, hf=None):
    if db_type == "faiss":
        return FAISS.load_local(f"{output_dir.split('log')[0]+'data'}/db/{name}_faiss3_general", hf, allow_dangerous_deserialization=True)
    elif db_type == "docs":
        with open(f"{output_dir.split('log')[0]+'data'}/db/{name}_docs3_general.pkl", "rb") as f:
