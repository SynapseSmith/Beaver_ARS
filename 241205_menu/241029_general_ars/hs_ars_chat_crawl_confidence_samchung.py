...
class args:
    ...
    instruction_template = "/home/user09/beaver/data/shared_files/prompt/incontext_v8_hs.txt"
...
def read_db(output_dir, db_type, name, hf=None):
    if db_type == "faiss":
        return FAISS.load_local(f"{output_dir.split('log')[0]+'data'}/db/{name}_faiss4_general", hf, allow_dangerous_deserialization=True)
    elif db_type == "docs":
        with open(f"{output_dir.split('log')[0]+'data'}/db/{name}_docs4_general.pkl", "rb") as f:
...
if __name__ == '__main__':
    app.run(host='0.0.0.0', port=6000, debug=False)   #5000포트에서 요청을 받음. 따라서 클라이언트에서는 해당 로컬 서버의 IP주소에서 포트 넘버 5006으로 보내야함.