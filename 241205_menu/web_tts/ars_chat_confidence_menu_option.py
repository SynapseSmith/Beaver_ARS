...
class args:
    store = '103017'   # 103017(미스), 104049(홍콩), 104562(새마을), 104570(롤링), 105200(우아)
...
def read_db(output_dir, db_type, name, hf=None):
    if db_type == "faiss":
        return FAISS.load_local(f"{output_dir.split('log')[0]+'data'}/db/{name}_faiss_menuoption_v1", hf, allow_dangerous_deserialization=True)
    elif db_type == "docs":
        with open(f"{output_dir.split('log')[0]+'data'}/db/{name}_docs_menuoption_v1.pkl", "rb") as f:
