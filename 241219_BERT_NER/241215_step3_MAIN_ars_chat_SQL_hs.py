...
class Args:
    def __init__(self):
        ...
        self.output_dir = "/home/user09/beaver/data/shared_files/241219_BERT_NER/checkpoint/klue_roberta_large_v2"
        self.data_path = "/home/user09/beaver/data/shared_files/241219_BERT_NER/data/dataset_SQL_general_ju_4_hong_preprocessed.xlsx"
        ...
        self.response_templates = {
            ...
            2: [
                "더 자세한 메뉴 정보 제공을 위해 QR URL 주소를 전송해드릴게요."
            ],
            3: [
            ...
            9: [
                "저희 매장에서는 {SPICY_MENU} 메뉴는 {INDI_TYPE_NM2} 메뉴입니다."
            ],
            10: [
            ...
            14: [
                "해당 문의는 확인 후 전화하신 번호로 안내드릴게요."
            ],
            15: [
                "저희 매장의 영업시간은 {SALE_BGN_TM}부터 {SALE_END_TM}까지에요."
            ],
            16: [
                "저희 매장에서는 오픈 시간이 {SALE_BGN_TM}, 종료 시간이 {SALE_END_TM}입니다."
            ],
            17: [
                "저희 매장에서는 브레이크 타임이 {STORE_REST_BGN_TM}부터 {STORE_REST_END_TM}까지 입니다."
            ],
            18: [
                "저희 매장에서 브레이크 타임은 {STORE_REST_BGN_TM}에 시작해서 {STORE_REST_END_TM}에 끝납니다."
            ],
            19: [
                "저희 매장의 휴일은 {HOLIDAY_TYPE_CD}이에요."
            ],
            20: [
            ...
            22: [
                "저희 매장은 {DAY_NM}에 영업 {DAY_INFO}해요. {SALE_BGN_TM}부터 {SALE_END_TM} 사이에 방문해주세요."
            ],
            23: [
            ...
            39: [
                "저희 매장에서는 {USE_MIN_ORDER_AMT}원 이상 구매 시 쿠폰 사용이 가능합니다."
                ],
            40: [
                "멤버십 관련한 자세한 문의는 확인 후 전화하신 번호로 안내 드릴게요."
                ],
            41: [
                "현재 저희 매장에서는 {EVENT_NM} 이벤트가 진행되고 있어요."
                ],
            42: [
                "{EVENT_NM} 이벤트는 {EVENT_BNEF_CND_CD}동안 {STD_MENU_NM}에 대한 {OFR_DC_VAL} 할인하고 있어요."
                ],   # 슬롯
            43: [
                "해당 문의는 확인 후 전화하신 번호로 안내 드릴게요."
                ],
...
def execute_sql(intent_id, df):
    try:
        ...
        elif intent_id == 42:   # EVENT_NM, EVENT_BNEF_CND_CD, STD_MENU_NM, OFR_DC_VAL
            result = df.sample(frac=1, random_state=None).query("EVENT_NM != '없음' and EVENT_BNEF_CND_CD != '없음' and OFR_DC_VAL != '없음'")[['STORE_NM','EVENT_NM', 'EVENT_BNEF_CND_CD', 'OFR_DC_VAL']].rename(columns={'STD_PROD_NM': 'STD_MENU_NM'}).drop_duplicates()
            
        elif intent_id in [2, 14, 21, 25, 27, 29, 30, 34, 35, 40, 43, 44, 45, 46, 47]:  # 고정 응답이 필요한 intent_id
            print(f"Static response required for intent_id {intent_id}")
            return None
        
        elif intent_id == [1, 13, 22]:  # 슬롯 채울 넘버들
            return None
...
holiday_info = {
    '월요일': {'영업여부': '', '영업시작시간': '11시시', '영업종료시간': '20시 30분'},  # !!!!!!!!!!! 안하는 경우에는 영업 시작 시간 부분에 영업하는 요일 붙이기!!
...
def order():
    ...
    try:
        ...
        if intent_id in [1, 13, 22]:
            ...
            if intent_id == 13:  # 가능 여부 판단을 위해서 임계값 이상인 게 있는 경우는 있다고 판단, 없는 경우 없다고 판단.
