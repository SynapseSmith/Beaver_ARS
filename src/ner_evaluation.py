...
parser.add_argument("--test_data", type=str, required=True, help="Path to test data in CoNLL format")
parser.add_argument("--output_xlsx", type=str, default="ner_test_results.xlsx", help="Output Excel file")
...
# 데이터 읽기
sentences, labels = read_conll(args.test_data)
...
# 모델 로드
model = AutoModelForTokenClassification.from_pretrained(args.model_path, num_labels=len(label_list))
...
# 엑셀 저장
results_df.to_excel(args.output_xlsx, index=False)
print(f"Test results saved to {args.output_xlsx}")
